...
const UploadProduct = ({ onclose,fetchData }) => {
  ...
  const handleUploadProduct = async (e) => {
    ...
    setData((prev) => ({
      ...prev,
      productImage: [...prev.productImage, uploadImageCloudinary.url], // Spread the previous productImage array
    }));
  };
  ...
  const handleDeleteProductImage=async(index)=>{
    ...
    setData((prev) => ({
      ...prev,
      productImage: [...newProductImage], // Spread the previous productImage array
    }));
  };
    ...
    <div className="fixed w-full h-full bg-slate-200 bg-opacity-35 bottom-0 top-0 left-0 right-0 flex justify-center items-center">
      <div className="bg-white p-4 rounded w-full max-w-2xl h-full max-h-[80%] overflow-hidden">
        ...
        <option value={""} >Select Category</option>
          ...
          <div>
            {data.productImage.length > 0 ? (
              ...
              <img
                  ...
                  />
                  <div className="absolute bottom-0 right-0 cursor-pointer p-1 text-white bg-red-500 rounded-full hidden group-hover:block" onClick={()=>handleDeleteProductImage(index)}>
                  <MdDelete />
                  </div>
              </div>
                ))}
          ...
          <textarea onChange={handleOnChange} name="description" value={data.description} id="description" className="h-28 p-1  bg-slate-100 border resize-none" placeholder="Enter product Description" rows={3} ></textarea>
